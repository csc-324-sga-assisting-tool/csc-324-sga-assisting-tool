...
function BudgetStatusDisplay({ status }: { status: Status }) {
  ...
  return (
    <div>
      ...
      <span className={`px-2 w-fit items-center justify-center text-md font-medium text-${color}-800 bg-${color}-100 rounded-full dark:bg-${color}-700 dark:text-${color}-500 float-right`}>{text}</span>
    </div>
  );
...
function DateDisplay({ dateISO }: { dateISO: string }) {
  ...
  return (
    <span className="text-md dark:text-gray-100 text-black float-right">{month} {day}, {year}</span>
  )
}
  ...
  return (
    <div className="grid grid-cols-6 gap-4 bg-white shadow-md rounded-lg p-4 m-5 ml-72 divide-x divide-solid max-w-3xl">
      <div className="h-full items-center justify-center col-span-1">
        <span className="text-xl text-bold text-black justify-center h-full w-full p-2"> {props.title} </span>
      </div>
      ...
      <div className="col-span-2 p-2">
        ...
        <span className="text-lg text-black float-right text-bold">$ {props.total}</span>
      </div>
